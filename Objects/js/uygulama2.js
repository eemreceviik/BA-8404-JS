...
// Console'a companyName - contactName - street yazdır

// suppliers.forEach((supplier) => {
//     console.log(
//         `Company Name: ${supplier.companyName} | Supplier Name:  ${supplier.contactName} `
//     );
//     console.log('Street: ' + supplier.address.street);
// });

// Ülkesi japonya olan kaç supplier var?
// const result = suppliers.filter(
//     (supplier) => supplier.address.country === 'Australia'
// ).length;

// console.log(result);

// Telefon numarası 514 ile başlayan kaç müşterim var? (semboller temizlenecek)

const result = suppliers.filter((supplier) => {
    let phone = supplier.address.phone;
    ...
    if (formattedPhone.length > 3) {
        firstThreeChar = formattedPhone.slice(0, 3);

        if (firstThreeChar === '514') {
            return true;
        }
    }
});
